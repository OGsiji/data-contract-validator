...
class CICDValidator:
    ...
    def validate_for_cicd(self, mode: str, changed_files: List[str] = None) -> Dict[str, Any]:
        ...
        # 🚀 FIXED: Safely set fast_mode flag with proper error handling
        try:
            # Make sure the config structure exists
            if 'data_source' not in self.config:
                self.config['data_source'] = {'type': 'dbt', 'config': {'project_path': '.'}}
            
            if 'config' not in self.config['data_source']:
                self.config['data_source']['config'] = {'project_path': '.'}
            
            # Now safely set fast_mode
            if mode == ValidationMode.FAST:
                self.config['data_source']['config']['fast_mode'] = True
                print("   ⚡ Fast mode enabled: Skipping DBT compilation")
            else:
                self.config['data_source']['config']['fast_mode'] = False
        ...
        try:
            ...
            validator.config = self.config  # This passes the fast_mode flag
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "version": "1.0",
            "data_source": {"type": "dbt", "config": {"project_path": "."}},
            "api_framework": {
                "type": "fastapi",
...
def output_for_github_actions(result: Dict[str, Any]):
    ...
    if result["success"]:
        ...
        print(
            f"::notice::Validation completed in {result['duration']:.2f}s - {result['summary']}"
        )
    else:
...
Examples:
  ...
  retl-validator init                        # Initialize configuration
    ...
    validate_parser.add_argument(
        "--config", default=".retl-validator.yml", help="Configuration file path"
    )
    validate_parser.add_argument(
        "--mode", choices=["fast", "diff", "full"], help="Validation mode"
    )
    validate_parser.add_argument(
        "--output-format", choices=["terminal", "github", "json"], default="terminal"
    )
    validate_parser.add_argument(
    ...
    init_parser.add_argument(
        "--auto-detect", action="store_true", help="Auto-detect project structure"
    )
    ...
    if args.command == "validate":
        ...
        result = validator.validate_for_cicd(
            mode=context["mode"], changed_files=context.get("changed_files", [])
        )
...
def auto_detect_config() -> Dict[str, Any]:
    ...
    config = {"version": "1.0", "name": "auto-detected-contracts"}

    # Detect DBT project
    if Path("dbt_project.yml").exists():
        config["data_source"] = {"type": "dbt", "config": {"project_path": "."}}
...
def get_default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "data_source": {"type": "dbt", "config": {"project_path": "./dbt_project"}},
        "api_framework": {"type": "fastapi", "config": {"models_module": "app.models"}},
        "validation": {
            "mode": "strict",
            "rules": {
                "missing_tables": "error",
