...
class YAMLContractValidator:
    ...
    def validate_contracts(self) -> Tuple[bool, List[ValidationIssue]]:
        ...
        try:
            ...
            critical_issues = [i for i in self.issues if i.severity == ValidationSeverity.ERROR]
    ...
    def _load_fastapi_schema(self) -> Dict[str, Any]:
        ...
        if not self.fastapi_yaml_path.exists():
            raise FileNotFoundError(f"FastAPI YAML file not found: {self.fastapi_yaml_path}")
    ...
    def _validate_schema_compatibility(self, 
                                     fastapi_schemas: Dict[str, Any], 
                                     dbt_schemas: Dict[str, Any]):
        ...
        for table in missing_in_dbt:
            ...
            suggestion = f"Did you mean: {', '.join(similar_tables)}?" if similar_tables else "Create this table in your DBT models"
            ...
            print(f"   🚨 CRITICAL: Missing table '{table}' needed by FastAPI models: {dependent_models}")
        
        # Extra DBT tables are not critical for now
        extra_in_dbt = dbt_table_names - fastapi_table_names
        if extra_in_dbt:
            print(f"   ℹ️  DBT has {len(extra_in_dbt)} extra tables not used by FastAPI: {list(extra_in_dbt)[:3]}...")
    ...
    def _validate_column_compatibility(self, 
        ...
        for table_name, fastapi_schema in fastapi_schemas.items():
            ...
            for col_name, col_info in fastapi_columns.items():
                if col_name not in dbt_columns:
                    if col_info['required']:
                        ...
                        similar_cols = self._find_similar_column_names(col_name, dbt_columns.keys())
                        suggestion = f"Did you mean: {', '.join(similar_cols)}?" if similar_cols else "Add this column to your DBT model"
            ...
            if missing_required:
                print(f"   🚨 CRITICAL: Table '{table_name}' missing {len(missing_required)} REQUIRED columns: {missing_required}")
    ...
    def _find_similar_table_names(self, target: str, candidates: Set[str]) -> List[str]:
        ...
        for candidate in candidates:
            ...
            if abs(len(target_lower) - len(candidate_lower)) <= 2:
                ...
                if (target_lower.startswith(candidate_lower[:3]) or 
                    candidate_lower.startswith(target_lower[:3]) or
                    target_lower.endswith(candidate_lower[-3:]) or
                    candidate_lower.endswith(target_lower[-3:])):
                    similar.append(candidate)
        
        return similar[:3]  # Return top 3 matches

    def _find_similar_column_names(self, target: str, candidates: Set[str]) -> List[str]:
        ...
        for candidate in candidates:
            ...
            # Simple similarity checks
            if abs(len(target_lower) - len(candidate_lower)) <= 2:
                if (target_lower.startswith(candidate_lower[:2]) or 
                    candidate_lower.startswith(target_lower[:2])):
                    similar.append(candidate)
    ...
    def generate_report(self, include_suggestions: bool = True) -> str:
        ...
        critical_errors = [i for i in self.issues if i.severity == ValidationSeverity.ERROR]
        minor_warnings = [i for i in self.issues if i.severity == ValidationSeverity.WARNING]
        ...
        if critical_errors:
            ...
            report.extend([
                "⚠️  YOU MUST FIX THESE BEFORE DEPLOYING! ⚠️",
                ""
            ])
        ...
        if minor_warnings and len(critical_errors) == 0:  # Only show if no critical issues
            report.extend([
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("")
        print("Example:")
        print("  python yaml_contract_validator.py pydantic_generated_contract.yml dbt_generated_contract.yml")
        sys.exit(1)
    ...
    try:
        ...
        if critical_issues:
            ...
            for issue in critical_issues:
                print(f"   💥 {issue.table}: FastAPI needs this but DBT doesn't provide it")
