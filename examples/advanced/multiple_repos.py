...
def validate_multiple_apis():
    ...
    apis = [
        (
            "user-service",
            FastAPIExtractor.from_github_repo(
                repo="my-org/user-service", path="app/models.py"
            ),
        ),
        (
            "order-service",
            FastAPIExtractor.from_github_repo(
                repo="my-org/order-service", path="src/models.py"
            ),
        ),
        (
            "analytics-api",
            FastAPIExtractor.from_local_file("./analytics-api/models.py"),
        ),
    ]
    ...
    for service_name, api_extractor in apis:
        ...
        print(
            f"   {service_name}: {'✅' if result.success else '❌'} "
